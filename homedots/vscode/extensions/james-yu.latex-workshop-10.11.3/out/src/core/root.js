...
function findFromRoot() {
    ...
    if (langId && (lw_1.lw.file.hasLaTeXLangId(langId) || lw_1.lw.file.hasLaTeXClassPackageLangId(langId))
        && lw_1.lw.cache.getIncludedTeX().has(vscode.window.activeTextEditor.document.fileName)) {
        logger.log(`Current root file is still valid: ${exports.root.file.path}`);
...
async function findFromActive() {
    ...
    if (lw_1.lw.file.hasAlwaysRootExt(path.extname(activeFilePath))) {
        ...
        for (const fileUri of fileUris) {
            ...
            if (result) {
                ...
                if (vscode.window.activeTextEditor
                    ...
                    && lw_1.lw.cache.getIncludedTeX(fileUri.fsPath).has(activeFilePath)) {
