...
const globals_1 = __webpack_require__(3);
const sideview_1 = __webpack_require__(5);
const python_extension_1 = __webpack_require__(126);
async function activate(context) {
    ...
    const sideview = new sideview_1.ManimSideview(context, pythonApi);
    context.subscriptions.push(vscode.commands.registerCommand("manim-sideview.run", (...args) => sideview.cmdRun(...args)), vscode.commands.registerCommand("manim-sideview.removeAllJobs", () => sideview.cmdRemoveAllJobs()), vscode.commands.registerCommand("manim-sideview.stop", () => sideview.cmdStop()), vscode.commands.registerCommand("manim-sideview.renderNewScene", (...args) => sideview.cmdRenderNewScene(...args)), vscode.commands.registerCommand("manim-sideview.removeCurrentJob", () => sideview.cmdRemoveJob()), vscode.commands.registerCommand("manim-sideview.showMobjectGallery", () => sideview.gallery.show()), vscode.commands.registerCommand("manim-sideview.syncMobjectGallery", () => sideview.gallery.synchronize(true)), vscode.commands.registerCommand("manim-sideview.updateDefaultManimConfig", () => sideview.cmdUpdateDefaultManimConfig()), vscode.commands.registerCommand("manim-sideview.showOutputChannel", () => globals_1.LOGGER.show(true)), vscode.commands.registerCommand("manim-sideview.showExtensionManimConfig", () => vscode.workspace
    ...
    vscode.window.onDidChangeTextEditorSelection((e) => sideview.auditTextEditorChange(e.textEditor), null, context.subscriptions);
...
function getVideoOutputPath(config, extension = ".mp4") {
    if (config.manimConfig.pixel_height && config.manimConfig.pixel_width) {
        ...
        if (!Object.keys(exports.DEFAULT_CONFIG.quality_map).includes(config.manimConfig.quality)) {
            vscode.window.showErrorMessage(Log.error(`Manim Sideview: Quality set as "${config.manimConfig.quality}" is invalid. It must be one of [${Object.keys(exports.DEFAULT_CONFIG.quality_map).join(", ")}].`));
            throw new Error("Invalid quality provided.");
        }
        var quality = exports.DEFAULT_CONFIG.quality_map[config.manimConfig.quality];
    }
...
function getImageOutputPath(config, loggedImageName, extension = ".png") {
    return insertContext({
        ...
        "{extension}": extension,
    }, path.join(config.manimConfig.images_dir, loggedImageName || exports.DEFAULT_CONFIG.image_name));
}
...
function updateFallbackManimCfg(updated, saveUpdated = true) {
    Object.keys(exports.DEFAULT_CONFIG).forEach((ky) => {
        if (updated[ky]) {
            exports.DEFAULT_CONFIG[ky] = updated[ky];
        }
    });
    if (saveUpdated) {
        fs.writeFile(exports.PATHS.cfgMap.fsPath, JSON.stringify(exports.DEFAULT_CONFIG), () => { });
    }
}
...
const path = __importStar(__webpack_require__(1));
const fs = __importStar(__webpack_require__(4));
const configparser_1 = __importDefault(__webpack_require__(6));
const child_process_1 = __webpack_require__(11);
const globals_1 = __webpack_require__(3);
const job_1 = __webpack_require__(12);
const player_1 = __webpack_require__(13);
const gallery_1 = __webpack_require__(15);
const pseudoTerm_1 = __webpack_require__(125);
...
function quoteSpecialChars(text) {
    // special characters
...
class ManimSideview {
    ctx;
    pythonApi;
    constructor(ctx, pythonApi) {
        this.ctx = ctx;
        this.pythonApi = pythonApi;
        this.ctx = ctx;
        this.pythonApi = pythonApi;
        this.jobManager = new job_1.JobStatusManager();
    ...
    async cmdRun(srcPath, autoRun) {
        ...
        if (manimConfig) {
            ...
            if (!activeJob || !activeJob.config.isUsingConfFile) {
                ...
                vscode.window.showInformationMessage(globals_1.Log.info("Manim Sideview: Loaded a configuration file from the working directory."));
            }
        }
        else {
            isConfFile = false;
            manimConfig = (0, globals_1.getDefaultConfig)();
        }
        let currentRunningConfig;
        if (activeJob) {
            ...
            globals_1.Log.info(`Asked user for a new scene name and recieved "${newSceneName}".`);
    ...
    async cmdRemoveJob(srcPath) {
        globals_1.Log.info(`Removing job for file ${srcPath}.`);
        ...
        if (job) {
            ...
            globals_1.Log.info(`No job found for file ${srcPath}.`);
        }
    }
    refreshJobStatus(srcPath) {
        globals_1.Log.info(`Refreshing job status.`);
    ...
    async cmdRenderNewScene(runningCfgSrcPath) {
        console.log("cmdRenderNewScene", runningCfgSrcPath);
        const job = this.jobManager.getActiveJob(runningCfgSrcPath);
        if (!job) {
            vscode.window.showErrorMessage(globals_1.Log.error("Manim Sideview: Select a Python file first to render a new scene!"));
            return;
        }
    ...
    async getRenderSceneName(srcFileUri) {
        globals_1.Log.info(`Fetching the scene name for probably render file ${srcFileUri}.`);
    ...
    async getManimPath() {
        let manimPath = path.normalize((0, globals_1.getUserConfiguration)("defaultManimPath"));
        let envName = null;
        globals_1.Log.info(`Default manim path is found as "${manimPath}"`);
        if (!path.isAbsolute(manimPath)) {
            const env = await this.getPythonEnvironment();
            if (env) {
                envName = env.name || "base";
                globals_1.Log.info(`Using python environment "${envName}" for manim.`);
                let bin = PYTHON_ENV_SCRIPTS_FOLDER[process.platform];
                if (!bin) {
                    globals_1.Log.error("Manim Sideview: Unsupported platform for python environment. Assuming linux directory.");
                ...
                const isPythonPath = ["\\bin\\python", "\\Scripts\\python.exe"].some((execPath) => normalizedPath.endsWith(execPath));
                let pythonDir;
                if (isPythonPath) {
                    pythonDir = path.dirname(env.folderUri.fsPath);
                    globals_1.Log.info(`Using Python executable directory: ${pythonDir}`);
                }
                else {
                    pythonDir = path.join(env.folderUri.fsPath, bin);
                }
                manimPath = path.join(pythonDir, manimPath);
                globals_1.Log.info(`Resolved manim path: ${manimPath} (from environment: ${env.folderUri.fsPath})`);
            }
        }
        // Check if manim path exists
        if (!(await this.checkManimExists(manimPath))) {
            if (await this.checkManimExists("manim")) {
                vscode_1.window.showWarningMessage(globals_1.Log.warn(`Manim Sideview: Executable not found at ${manimPath}, but found executable on PATH...`));
                manimPath = "manim";
            }
            else {
                const msg = globals_1.Log.error(`Manim Sideview: Manim is not found in PATH or at the specified location "${manimPath}". Please ensure manim is installed correctly or specify a valid path in settings.`);
                vscode_1.window.showErrorMessage(msg, "Go to Settings").then((selection) => {
                    if (selection === "Go to Settings") {
                        vscode.commands.executeCommand("workbench.action.openSettings", "manim-sideview.defaultManimPath");
                    }
                });
                throw Error(msg);
            }
    ...
    async cmdUpdateDefaultManimConfig() {
        ...
        const process = (0, child_process_1.spawn)((await this.getManimPath()).manim, ["cfg", "show"]);
        let fullStdout = "";
     ...
     */
    executeTerminalCommand(outputPath, srcPath, sceneName, cwd) {
        const terminalCommand = (0, globals_1.getUserConfiguration)("terminalCommand");
        const commandInput = (0, globals_1.insertContext)({
        ...
        globals_1.Log.info(`Executing post-render command "${commandInput}" in terminal "${cli.name}".`);
        cli.sendText(commandInput);
    }
    /**
     * Gets the output channel and configures it to the cwd.
     */
    getOutputChannel(cwd, envName) {
        let outputChannel;
        if ((0, globals_1.getUserConfiguration)("outputToTerminal")) {
            this.ensureOutputChannel((0, globals_1.getUserConfiguration)("outputToTerminal"));
     ...
     */
    async render(config) {
        globals_1.Log.info("Attempting to render via the running configuration " +
        ...
        this.jobManager.addJob(config, player_1.PlayableMediaType.Video);
        const args = [
            config.srcPath,
        ...
        this.spawnManimProcess(manim.manim, args, cwd, config.srcPath, config.sceneName, (mediaInfo) => {
            globals_1.Log.info(`Running process for "${config.sceneName}" has finished.`);
            const mediaPath = mediaInfo.fileType === player_1.PlayableMediaType.Video
                ? (0, globals_1.getVideoOutputPath)(config)
                : (0, globals_1.getImageOutputPath)(config, mediaInfo.imageName);
            const filePath = vscode.Uri.file(path.join(config.srcRootFolder, mediaPath));
            globals_1.Log.info(`Predicted output file path is "${filePath.fsPath}" for "${config.sceneName}".`);
            if (!fs.existsSync(filePath.fsPath)) {
                vscode.window
                    .showErrorMessage(globals_1.Log.error(`Manim Sideview: Predicted output file does not exist at "${filePath.fsPath}"` +
            ...
            if ((0, globals_1.getUserConfiguration)("preview")) {
     ...
     * @returns
     */
    async spawnManimProcess(command, args, cwd, srcPath, sceneName, onProcessClose) {
        ...
        process.stderr.on("data", (data) => {
            ...
            globals_1.Log.warn(`[${process.pid}] Captured stderr output "${formatOutput(dataStr)}"`);
        ...
        process.stdout.on("data", (data) => {
            const dataStr = data.toString();
            globals_1.Log.info(`[${process.pid}] RE: "${formatOutput(dataStr)}"`);
            if (!process.killed) {
                ...
                if (stdoutLogbook.includes(KILL_MSG)) {
                    ...
                    this.outputChannel.append("\r\n" +
                        globals_1.Log.error(`[${process.pid}] Your selected scene name does not exist in the source file.`) +
                        "\r\n");
        ...
        process.on("close", async (code, signal) => {
            ...
            if (code !== 0) {
                if (code === -4058) {
                    vscode.window.showErrorMessage(globals_1.Log.error(`Manim Sideview: Unable to find the source file. Try opening the folder containing this file instead of a single file, or check file permissions.`));
                }
                else if (code != 15) {
                    // only show the error message if the process was not killed by us
                    vscode.window
                        .showErrorMessage(globals_1.Log.error(`Manim Sideview: Error rendering file (exit code ${code}). Check the output for more details.`), "Show Logs")
                        .then((selection) => {
            ...
            this.outputChannel.appendLine(globals_1.Log.info(`[${process.pid}] Execution returned code=${code} in ${timeElapsed} seconds ${code === 1 ? "returned signal " + signal : ""} ${signal === "SIGTERM"
        ...
        globals_1.Log.info(`[${process.pid}] Spawned a new process for executing "${commandString}".`);
    }
    /**
     ...
     * @returns
     */
    async getMediaFileInfo(stdoutLogbook, srcPath, sceneName) {
        let fileType;
        let imageName;
        const job = this.jobManager.getActiveJob(srcPath);
        globals_1.Log.info(`Attempting to determine the output file type for "${sceneName}".`);
        ...
        if (fileReSignifier.length > 0) {
            ...
            if (fileIdentifier) {
                fileType = player_1.PlayableMediaType.Image;
                ...
                fileType = player_1.PlayableMediaType.Video;
            }
            globals_1.Log.info(`[${process.pid}] Render output is predicted as "${fileType === player_1.PlayableMediaType.Image ? "Image" : "Video"}".`);
        }
        if (fileType === undefined) {
            if (job.runtimeOptions.outputFileType === undefined) {
                // we don't have a prior user input to determine the output type
                const inputFileType = await vscode.window.showWarningMessage(globals_1.Log.warn(`Manim Sideview: Unable to infer the output filetype for "${sceneName}". Please select one below!`), "Video", "Image");
                if (!inputFileType) {
        ...
        globals_1.Log.info(`File type is set to "${fileType}".`);
        return { fileType: fileType || player_1.PlayableMediaType.Video, imageName };
    }
    /**
     ...
     */
    getPreferenceArgs() {
        const cmdLineArgs = (0, globals_1.getUserConfiguration)("commandLineArgs");
     ...
     */
    ensureOutputChannel(pseudoTerm) {
        if (pseudoTerm) {
            if (!this.manimPseudoTerm || this.manimPseudoTerm.isClosed()) {
                this.manimPseudoTerm = new pseudoTerm_1.ManimPseudoTerm("manim");
            }
        }
    ...
    async getManimConfigFile(srcfilePath) {
        ...
        globals_1.Log.info(`Parsing configuration file "${filePath}".`);
        try {
            var parsedConfig = new configparser_1.default();
            await parsedConfig.readAsync(filePath);
        }
        catch (e) {
            vscode.window.showErrorMessage(globals_1.Log.error(`Manim Sideview: Error whilst parsing manim.cfg file, ignoring it. ${e}`));
            return;
        }
        ...
        let manimConfig = (0, globals_1.getDefaultConfig)();
        for (const flag of RELEVANT_CONFIG_OPTIONS) {
            if (parsedConfig.hasKey(CONFIG_SECTION, flag)) {
                manimConfig[flag] = parsedConfig.get(CONFIG_SECTION, flag);
                globals_1.Log.info(`Set flag "${flag}" to ${parsedConfig.get(CONFIG_SECTION, flag)}.`);
            }
        }
        return manimConfig;
    }
     ...
     */
    createRunningConfig(document, sceneName, isUsingCfgFile, manimConfig) {
        const srcPath = document.uri.fsPath;
        globals_1.Log.info(`Creating a new running configuration for file "${srcPath}"`);
    ...
    async checkManimExists(manimPath) {
        try {
            ...
            (0, child_process_1.execSync)(`${checkCommand} ${manimPath}`);
            return true;
        }
        catch (error) {
            return false;
...
const path = __webpack_require__(1);
const mkdirp = __webpack_require__(8);
const errors = __webpack_require__(9);
const interpolation = __webpack_require__(10);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.JobStatusManager = void 0;
const vscode = __importStar(__webpack_require__(2));
const globals_1 = __webpack_require__(3);
/**
...
class JobStatusManager {
    ...
    getActiveJob(srcPath) {
        if (srcPath) {
            ...
            if (!editor || editor.document.languageId !== "python") {
                globals_1.Log.info(`No active editor or language is not python.`);
                return null;
            }
            globals_1.Log.info(`Getting active job for file ${editor.document.fileName}. Active Jobs: ${Object.keys(this.activeJobs)}`);
    ...
    setRunning(activeJob) {
        if (activeJob) {
            activeJob.status = 2 /* JobStatus.running */;
        }
    ...
    setActive(activeJob) {
        if (activeJob) {
            activeJob.status = 1 /* JobStatus.active */;
        }
        this.jobStatusItem.color = new vscode.ThemeColor("textLink.foreground");
    ...
    setError(activeJob) {
        if (activeJob) {
            activeJob.status = 0 /* JobStatus.error */;
        }
...
const vscode = __importStar(__webpack_require__(2));
const globals_1 = __webpack_require__(3);
const templateEngine_1 = __webpack_require__(14);
...
class MediaPlayer {
    ...
    async playMedia(mediaUri, config, mediaType) {
        if (this.recentMediaPanel) {
            ...
            globals_1.Log.info(`Reloading media to URI "${mediaUri.fsPath}"`);
        ...
        let videoHideState;
        let imageHideState;
        if (mediaType === exports.PlayableMediaType.Video) {
        ...
        panel.webview.html = await engine.render({
            ...
            imageHideState,
            videoHideState,
            background: (0, globals_1.getUserConfiguration)("checkeredBackground")
                ? "checkered-bg"
                : "",
            outputFile: mediaUri.fsPath,
            sourceFile: config.srcPath,
            ...
            autoplay: (0, globals_1.getUserConfiguration)("previewAutoPlay") ? "autoplay muted" : "",
        });
        globals_1.Log.info(`Playing media URI "${mediaUri.fsPath}"`);
        panel.webview.onDidReceiveMessage((message) => {
            switch (message.command) {
                // Executes a manim-sideview command and then executes run command
                case "executeSelfCommand":
                    globals_1.Log.info(`Executing command "manim-sideview.${message.name}" for webview.`);
...
const path = __importStar(__webpack_require__(1));
const yaml = __importStar(__webpack_require__(16));
...
var composer = __webpack_require__(17);
var Document = __webpack_require__(21);
var Schema = __webpack_require__(38);
var errors = __webpack_require__(63);
var Alias = __webpack_require__(22);
var identity = __webpack_require__(19);
var Pair = __webpack_require__(30);
var Scalar = __webpack_require__(29);
var YAMLMap = __webpack_require__(40);
var YAMLSeq = __webpack_require__(43);
var cst = __webpack_require__(79);
var lexer = __webpack_require__(83);
var lineCounter = __webpack_require__(84);
var parser = __webpack_require__(85);
var publicApi = __webpack_require__(86);
...
var directives = __webpack_require__(18);
var Document = __webpack_require__(21);
var errors = __webpack_require__(63);
var identity = __webpack_require__(19);
var composeDoc = __webpack_require__(64);
var resolveEnd = __webpack_require__(74);
...
var identity = __webpack_require__(19);
var Pair = __webpack_require__(30);
var toJS = __webpack_require__(26);
var Schema = __webpack_require__(38);
var stringifyDocument = __webpack_require__(62);
var anchors = __webpack_require__(23);
var applyReviver = __webpack_require__(25);
var createNode = __webpack_require__(28);
var directives = __webpack_require__(18);
...
var createNode = __webpack_require__(28);
var stringifyPair = __webpack_require__(31);
var addPairToJSMap = __webpack_require__(36);
var identity = __webpack_require__(19);
...
var stringifyComment = __webpack_require__(33);
...
var identity = __webpack_require__(19);
var stringifyComment = __webpack_require__(33);
var stringifyString = __webpack_require__(34);
...
var Scalar = __webpack_require__(29);
var foldFlowLines = __webpack_require__(35);
...
var seq = __webpack_require__(42);
var string = __webpack_require__(44);
var tags = __webpack_require__(45);
...
var stringifyCollection = __webpack_require__(41);
var addPairToJSMap = __webpack_require__(36);
var Collection = __webpack_require__(27);
var identity = __webpack_require__(19);
var Pair = __webpack_require__(30);
...
var map = __webpack_require__(39);
var _null = __webpack_require__(46);
var seq = __webpack_require__(42);
var string = __webpack_require__(44);
var bool = __webpack_require__(47);
var float = __webpack_require__(48);
var int = __webpack_require__(50);
var schema = __webpack_require__(51);
var schema$1 = __webpack_require__(52);
var binary = __webpack_require__(53);
var omap = __webpack_require__(54);
var pairs = __webpack_require__(55);
var schema$2 = __webpack_require__(56);
var set = __webpack_require__(60);
var timestamp = __webpack_require__(61);
...
var Scalar = __webpack_require__(29);
var stringifyNumber = __webpack_require__(49);
...
var string = __webpack_require__(44);
var binary = __webpack_require__(53);
var bool = __webpack_require__(57);
var float = __webpack_require__(58);
var int = __webpack_require__(59);
var omap = __webpack_require__(54);
var pairs = __webpack_require__(55);
var set = __webpack_require__(60);
var timestamp = __webpack_require__(61);
...
var Document = __webpack_require__(21);
var composeNode = __webpack_require__(65);
var resolveEnd = __webpack_require__(74);
var resolveProps = __webpack_require__(68);
...
var Alias = __webpack_require__(22);
var composeCollection = __webpack_require__(66);
var composeScalar = __webpack_require__(75);
var resolveEnd = __webpack_require__(74);
var utilEmptyScalarPosition = __webpack_require__(78);
...
var YAMLMap = __webpack_require__(40);
var YAMLSeq = __webpack_require__(43);
var resolveBlockMap = __webpack_require__(67);
var resolveBlockSeq = __webpack_require__(72);
var resolveFlowCollection = __webpack_require__(73);
...
var YAMLMap = __webpack_require__(40);
var resolveProps = __webpack_require__(68);
var utilContainsNewline = __webpack_require__(69);
var utilFlowIndentCheck = __webpack_require__(70);
var utilMapIncludes = __webpack_require__(71);
...
var identity = __webpack_require__(19);
var Scalar = __webpack_require__(29);
var resolveBlockScalar = __webpack_require__(76);
var resolveFlowScalar = __webpack_require__(77);
...
var cstScalar = __webpack_require__(80);
var cstStringify = __webpack_require__(81);
var cstVisit = __webpack_require__(82);
...
const FormData$1 = __webpack_require__(88);
const url = __webpack_require__(94);
const proxyFromEnv = __webpack_require__(109);
const http = __webpack_require__(92);
const https = __webpack_require__(93);
const util = __webpack_require__(7);
const followRedirects = __webpack_require__(110);
const zlib = __webpack_require__(123);
const stream = __webpack_require__(90);
const EventEmitter = __webpack_require__(124);
...
var CombinedStream = __webpack_require__(89);
var util = __webpack_require__(7);
var path = __webpack_require__(1);
var http = __webpack_require__(92);
var https = __webpack_require__(93);
var parseUrl = (__webpack_require__(94).parse);
var fs = __webpack_require__(4);
var Stream = (__webpack_require__(90).Stream);
var mime = __webpack_require__(95);
var asynckit = __webpack_require__(98);
var populate = __webpack_require__(108);
...
module.exports =
{
  parallel      : __webpack_require__(99),
  serial        : __webpack_require__(106),
  serialOrdered : __webpack_require__(107)
};
...
var iterate    = __webpack_require__(100)
  , initState  = __webpack_require__(104)
  , terminator = __webpack_require__(105)
  ;
...
var defer = __webpack_require__(102);
...
var abort = __webpack_require__(103)
  , async = __webpack_require__(101)
  ;
...
var serialOrdered = __webpack_require__(107);
...
module.exports = function () {
  if (!debug) {
    try {
      ...
      debug = __webpack_require__(113)("follow-redirects");
    }
    catch (error) { /* */ }
...
module.exports = __webpack_require__(115)(exports);
...
function setup(env) {
	...
	createDebug.humanize = __webpack_require__(116);
...
const tty = __webpack_require__(118);
const util = __webpack_require__(7);
...
try {
	...
	const supportsColor = __webpack_require__(119);
...
/* harmony export */ });
/* harmony import */ var node_process__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(120);
/* harmony import */ var node_os__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(121);
/* harmony import */ var node_tty__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(122);
...
class ManimPseudoTerm {
    ...
    envName = "";
    cwd = path.dirname(vscode.workspace.textDocuments[0]?.fileName || process.cwd());
    isRunning = false;
    writeEmitter = new vscode.EventEmitter();
    prompt = () => (this.envName ? `(${this.envName})` : defPrompt) + ` ${this.cwd}>`;
    intro = "Manim Extension XTerm\n\rServes as a terminal for logging purpose.\n\r\n\r" +
        `Extension Version ${globals_1.EXTENSION_VERSION}\n\r\n\r${this.prompt()}`;
    content = "";
    appendedBefore = false;
    stickyNotes = "";
    pty = {
